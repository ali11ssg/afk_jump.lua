        ...
        try:
            ...
            if not poll_for_receipt_id:
                for _retry in range(2):
                    ...
                    js_body2 = await fetch_actions_js(client, actions_url, shop_url)
                    poll_for_receipt_id = extract_poll_for_receipt_id(js_body2)
                    if poll_for_receipt_id:
                        break
            if not poll_for_receipt_id:
                raise Exception("could not extract poll_for_receipt_id from actions JS")
        except Exception as e:
            result.status = CheckStatus.ERROR
            result.retryable = True
